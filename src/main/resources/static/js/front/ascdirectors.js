 ...
 * */
function ascdirectorsList(pageNum) {
    // 데이터 조회
    searchPosts(pageNum);
...
function searchPosts(pageNum) {
    const countPerPage = 8; // 페이지당 노출 개수
    ...
    $.ajax({
        url: '/member/ascdirectors/selectList.do',
        method: 'post',
    ...
    .done(function (data, status){
        ...
        $.each(results , function(i){
            ...
            str += '<li>';
            if(lang === 'KO'){
                str += '<a href="/member/detailA.do?seq=' + seq + '">';
            }else{
