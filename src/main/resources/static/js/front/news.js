...
function searchPosts(pageNum) {
    ...
    $.ajax({
        url: '/community/news/selectList.do',
        method: 'post',
    ...
    .done(function (data, status){
        ...
        $.each(results , function(i){
            ...
            let fullFilePath = results[i].fullFilePath;
            if(nvl(fullFilePath, '') !== ''){
                thumbnailImage = fullFilePath.toString().replace('./tomcat/webapps', '../../..');
            }

            if(lang === 'KO'){
                str += '<li onclick="location.href=\'/community/news/detail.do?seq=' + seq + '\'">';
            }else{
                str += '<li onclick="location.href=\'/eng/community/news/detail.do?seq=' + seq + '\'">';
            }
                    str += '<div class="thumbBox thumb75">'
                        str += '<img src="' + thumbnailImage + '" class="thumbImg" alt="SIPA_NEWS_IMAGE_' + (i+1) + '">';
                    str += '</div>';
                    str += '<div class="txt_box">';
                        str += '<div class="tit">';
                            str += title;
                        str += '</div>';
                        str += '<div class="date">';
        ...
        if(nvl(results,"") !== "") {
            ...
            emptyStr += '<li>';
                emptyStr += '<div>';
                    ...
                    if(lang === 'KO'){
                        emptyStr += '조회된 SIPA-NEWS가 없습니다.';
                    }else{
                        emptyStr += 'No news have been retrieved for this condition.';
                    }
                    emptyStr += '</p>';
                emptyStr += '</div>';
            emptyStr += '</li>';
