 ...
 * */
function ascmembersList(pageNum) {
    // 데이터 조회
    searchPosts(pageNum);
...
function searchPosts(pageNum) {
    ...
    $.ajax({
        url: '/member/ascmembers/selectList.do',
        method: 'post',
    ...
    .done(function (data, status){
        ...
        $.each(results , function(i){
            ...
            str += '<li>';
            if(lang === 'KO'){
                str += '<a href="/member/detailC.do?seq=' + seq + '">';
            }else{
                str += '<a href="/eng/member/detailC.do?seq=' + seq + '">';
            }
                    str += '<div class="img thumb169 thumbBox">';
                        str += '<img src="' + logoFilePath + '" class="thumbImg" alt="회원_로고_이미지_' + (i+1) + '">';
                    str += '</div>';
                    str += '<div class="txt">';
                        str += '<div class="name">';
                            str += companyName;
