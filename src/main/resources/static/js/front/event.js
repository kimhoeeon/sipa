...
function searchPosts(pageNum) {
    ...
    let searchText = '';
    if(nvl(year, '') !== '' && nvl(month,'') !== ''){
        searchText = year + '-' + month;
    }
    ...
    $.ajax({
        url: '/community/event/selectList.do',
        method: 'post',
    ...
    .done(function (data, status){
        ...
        $.each(results , function(i){
            ...
            str += '<li>';
                str += '<div class="number">';
                    str += rownum;
                str += '</div>';
                str += '<div class="tit" style="cursor: unset">';
                    str += title;
                str += '</div>';
        ...
        $('.cal_list_box .tbody').empty();
        $('.cal_list_box .tbody').html(str);
        ...
        if(nvl(results,"") !== "") {
            ...
            emptyStr += '<li>';
                emptyStr += '<div>';
                    ...
                    if(lang === 'KO'){
                        emptyStr += '조회된 이벤트가 없습니다.';
                    }else{
                        emptyStr += 'No event have been retrieved for this condition.';
                    }
                    emptyStr += '</p>';
                emptyStr += '</div>';
            emptyStr += '</li>';
            ...
            $('.cal_list_box .tbody').empty();
            $('.cal_list_box .tbody').html(emptyStr);
        }
    })
