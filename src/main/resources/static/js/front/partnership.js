 ...
 * */
function partnershipList(pageNum) {
    // 데이터 조회
    searchPosts(pageNum);
...
function searchPosts(pageNum) {
    ...
    $.ajax({
        url: '/member/partnership/selectList.do',
        method: 'post',
    ...
    .done(function (data, status){
        ...
        $.each(results , function(i){
            ...
            str += '<li>';
            if(lang === 'KO'){
                str += '<a href="/member/detailE.do?seq=' + seq + '">';
            }else{
                str += '<a href="/eng/member/detailE.do?seq=' + seq + '">';
            }
                    str += '<div class="img thumb169 thumbBox">';
                        str += '<img src="' + logoFilePath + '" class="thumbImg thumbLogo" alt="협력사_로고_이미지_' + (i+1) + '">';
                    str += '</div>';
                    str += '<div class="txt">';
                        str += '<div class="name">';
                            str += companyName;
