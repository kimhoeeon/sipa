 ...
 * */
function consultationList(pageNum) {
    // 데이터 조회
    searchPosts(pageNum);
...
function searchPosts(pageNum) {
    ...
    $.ajax({
        url: '/member/consultation/selectList.do',
        method: 'post',
    ...
    .done(function (data, status){
        ...
        $.each(results , function(i){
            ...
            let organization = results[i].organization;
            let depart = results[i].depart;
            let homepage = results[i].homepage;
            let tel = results[i].tel;
            let briefDescription = results[i].briefDescription;

            let representFilePath = results[i].representFilePath;
            if(nvl(representFilePath,"") !== ""){
                representFilePath = representFilePath.toString().replace('./tomcat/webapps', '../../..');
            }else{
                representFilePath = '/static/img/img_adviser03.jpg';
            }

            str += '<li>';
            if(lang === 'KO'){
                str += '<a href="/member/detailB.do?seq=' + seq + '">';
            }else{
                str += '<a href="/eng/member/detailB.do?seq=' + seq + '">';
            }
                    str += '<div class="img thumb34 thumbBox">';
                        str += '<img src="' + representFilePath + '" class="thumbImg" alt="자문위원_이미지_' + (i+1) + '">';
                    str += '</div>';
                    str += '<div class="txt">';
                        str += '<div class="name">';
                        if(lang === 'KO'){
