 ...
 * @param cate
 * */
function issueList(pageNum, cate) {

    if(cate === '전체'){
        $('.sp_cate_list li').removeClass('on');
        $('.sp_cate_list li:first-child').addClass('on');
    }

    // 데이터 조회
    searchPosts(pageNum, cate);
 ...
 * @param cate
 */
function searchPosts(pageNum, cate) {
    ...
    $.ajax({
        url: '/support/issue/selectList.do',
        method: 'post',
    ...
    .done(function (data, status){
        ...
        $.each(results , function(i){
            ...
            let ministryCompetent = results[i].ministryCompetent;
            ...
            str += '<li>';
                ...
                str += '<div class="cate">';
                    str += supportField;
                str += '</div>';
                str += '<div class="tit" onclick="location.href=\'/support/issue/detail.do?seq=' + seq + '\'">';
                    str += supportBizName;
                str += '</div>';
                str += '<div class="pub">';
                    str += ministryCompetent;
                str += '</div>';
                str += '<div class="date">';
                    str += initRegiDttm;
                str += '</div>';
            ...
            emptyStr += '<li>';
                emptyStr += '<div>';
                    emptyStr += '<p style="font-size: 18px; font-weight: bold;">';
                        emptyStr += '조회된 Issue&Trend 가 없습니다.';
                    emptyStr += '</p>';
                emptyStr += '</div>';
            emptyStr += '</li>';
...
function f_issue_cate_search(target, category){
    $(target).parent().siblings().removeClass('on');
    $(target).parent().addClass('on');

    /* 조회 */
    issueList(1, category);
}