...
function f_company_bizanm_search(){
    ...
    let resData = ajaxConnect('/mng/company/bizanm/selectList.do', 'post', jsonObj);
    ...
    let jb = $('#mng_company_bizanm_table tbody td');
    let cnt = 0;
...
function f_company_bizanm_search_condition_init(){
...
function f_company_bizanm_detail_modal_set(seq){
    /* TM 및 잠재DB 목록 상세 조회 */
    let jsonObj = {
        seq: seq
    };

    let resData = ajaxConnect('/mng/company/bizanm/selectSingle.do', 'post', jsonObj);
...
function f_company_bizanm_remove(seq){
    //console.log('삭제버튼');
    if(nullToEmpty(seq) !== ""){
        let jsonObj = {
            seq: seq
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                let resData = ajaxConnect('/mng/company/bizanm/delete.do', 'post', jsonObj);

                if (resData.resultCode === "0") {
                    showMessage('', 'info', '사업공고 삭제', '사업공고가 삭제되었습니다.', '');
                    f_company_bizanm_search(); // 삭제 성공 후 재조회 수행
                } else {
                    showMessage('', 'error', '에러 발생', '사업공고 삭제를 실패하였습니다. 관리자에게 문의해주세요. ' + resData.resultMessage, '');
                }
            }
        });
    }
...
function f_company_bizanm_modify_init_set(seq){
    window.location.href = '/mng/company/bizanm/detail.do?seq=' + seq;
}

function f_company_bizanm_save(seq){
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            /* form valid check */
            let validCheck = f_company_bizanm_valid();
            ...
            if(validCheck){
                ...
                /* form data setting */
                let data = f_company_bizanm_form_data_setting();

                /* Modify */
                if(nvl(seq, '') !== ''){
                    $.ajax({
                        url: '/mng/company/bizanm/update.do',
                        method: 'POST',
                        async: false,
                        data: data,
                        dataType: 'json',
                        ...
                        success: function (data) {
                            if (data.resultCode === "0") {
                                Swal.fire({
                                    title: '사업공고 정보 변경',
                                    text: "사업공고 정보가 변경되었습니다.",
                                    icon: 'info',
                                    confirmButtonColor: '#3085d6',
                                    confirmButtonText: '확인'
                                }).then((result) => {
                                    if (result.isConfirmed) {
                                        f_company_bizanm_modify_init_set(seq); // 재조회
                                    }
                                });
                            } else {
                                showMessage('', 'error', '에러 발생', '사업공고 정보 변경을 실패하였습니다. 관리자에게 문의해주세요. ' + data.resultMessage, '');
                            }
                        },
                    ...
                    $.ajax({
                        url: '/mng/company/bizanm/insert.do',
                        method: 'POST',
                        async: false,
                        data: data,
                        dataType: 'json',
                        ...
                        success: function (data) {
                            if (data.resultCode === "0") {
                                Swal.fire({
                                    title: '사업공고 정보 등록',
                                    text: "사업공고 정보가 등록되었습니다.",
                                    icon: 'info',
                                    confirmButtonColor: '#3085d6',
                                    confirmButtonText: '확인'
                                }).then((result) => {
                                    if (result.isConfirmed) {
                                        window.location.href = '/mng/company/bizanm.do'; // 목록으로 이동
                                    }
                                });
                            } else {
                                showMessage('', 'error', '에러 발생', '사업공고 정보 등록을 실패하였습니다. 관리자에게 문의해주세요. ' + data.resultMessage, '');
                            }
                        },
                        error: function (xhr, status) {
                            alert('오류가 발생했습니다. 관리자에게 문의해주세요.\n오류명 : ' + xhr + "\n상태 : " + status);
...
function f_company_bizanm_form_data_setting(){
...
function f_company_bizanm_valid(){
    ...
    if(nvl(supportField,'') === ''){ showMessage('', 'error', '[등록 정보]', '지원분야를 선택해 주세요.', ''); return false; }
